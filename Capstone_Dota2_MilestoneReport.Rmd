...
## Dota 2 Capstone - Understanding the effect of team selection, hero selection and item purchase on Dota 2 gameplay
...
## Hero Stats
players_csv has detailed information about the player, the hero they chose and their in-game performance. This information is grouped by hero class, which are STR/INT/AGI. Further, all stats are normalized such that a particular stat of STR hero plus AGI hero plus INT hero equals one. This operation allows us to view all stats on a single heatmap. The code and the heatmap is shown below
```{r 4,echo=FALSE}
...
Linear regression, logistic regression and decision tree is used in the machine learning part of the capstone project. Linear regression was used to predict numeric values of trueskill_mu. Logistic regression was used to predict the probability of radiant or dire teams. Finally, a decision tree or CART is used to perform the same prediction as the logistic regression.  
...
Model accuracy using the training set comes out to (10368 + 23840)/(10368+20366+9302+23840), which equals 53.5%.

## Decision Tree

Decision tree model is used to predict the probability of radiant or dire teams winning.

```{r 23,echo=FALSE}
Combined_Tree_Train <- read.csv("Data/Combined_Tree_Train.csv")
Combined_Tree_Test <- read.csv("Data/Combined_Tree_Test.csv")
...
Final decision tree model gives an accuracy of (10466+12635)/(10466+2706+1569+12635), which equals 84.4%, substantially higher than the logistic regression.
...
3) Teams with >= 3 INT heroes have substantially higher probabilities of winning, followed by STR heroes. Team with >= 3 AGI heroes are at a very high disadvantage.
...
1) Given that duration is a significant variable in the logistic regression and is used twice as a splitter variable in the decision tree, it is important to ensure that game duration has no impact on game outcome. Logistic regression shows that duration has a negative co-efficient. So, lower duration games decrease radiant team`s probability of winning, giving dire teams an advantage. Instituting a bonus for radiant  teams in longer duration games can help overcome this bias.

2) trueskill_sigma is not a splitter variable in the decision tree. trueskill_sigma does not have an impact on the scatter plot between trueskill_mu and percentage_wins. Interestingly, trueskill_sigma is significant in the linear regression where trueskill_mu is the predictor. It has a negative co-efficient, showing that higher sigma lowers trueskill_mu. The trueskill algorithm requires review in the context of Dota 2. 

3) Teams with >=3 AGI heroes are at a significant disadvantage. A relook into the metrics of AGI heroes will be useful.