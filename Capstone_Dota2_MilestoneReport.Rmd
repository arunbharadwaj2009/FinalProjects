...
```
Below plot, drawn between item_1 and hero level, is very similar to the previous plot.
```{r 8,echo=FALSE}
function_boxplot_level_itemclass <- function(a){ggplot(players_csv2,aes(x=a,y=level)) + geom_boxplot() + theme(axis.text.x=element_text(angle=-90)) + labs(x="Item types bought by Hero",y="Hero level at game end")}
...
Below plot, drawn between item_2 and hero level, is very similar to the previous plot.
```{r 9,echo=FALSE}
function_boxplot_level_itemclass <- function(a){ggplot(players_csv2,aes(x=a,y=level)) + geom_boxplot() + theme(axis.text.x=element_text(angle=-90)) + labs(x="Item types bought by Hero",y="Hero level at game end")}
...
```
Below plot, drawn between item_3 and hero level, is very similar to the previous plot.
```{r 10,echo=FALSE}
function_boxplot_level_itemclass <- function(a){ggplot(players_csv2,aes(x=a,y=level)) + geom_boxplot() + theme(axis.text.x=element_text(angle=-90)) + labs(x="Item types bought by Hero",y="Hero level at game end")}
...
Below plot, drawn between item_4 and hero level, is a bit different since the level median of Dam (Damage) and Disable are nearly equal.
```{r 11,echo=FALSE}
function_boxplot_level_itemclass <- function(a){ggplot(players_csv2,aes(x=a,y=level)) + geom_boxplot() + theme(axis.text.x=element_text(angle=-90)) + labs(x="Item types bought by Hero",y="Hero level at game end")}
...
Below plot, drawn between item_5 and hero level, is similar to the first 4 plots.
```{r 12,echo=FALSE}
function_boxplot_level_itemclass <- function(a){ggplot(players_csv2,aes(x=a,y=level)) + geom_boxplot() + theme(axis.text.x=element_text(angle=-90)) + labs(x="Item types bought by Hero",y="Hero level at game end")}
...
It can be inferred that Damage and Disable class items have the highest positive effect on hero performance. Int item types have the lowest positive effect on hero performance. It is possible that players maximize hero performance by purchasing Damage or Disable items in all inventory slots (item_0 to item_5), thereby improving hero performance even further.
...
```
```{r 14,echo=FALSE}
item_and_hero <- players_csv2 %>% group_by(Class,item_1) %>% tally() 
item_and_hero <- item_and_hero %>% group_by(Class) %>% mutate(percentage=n/sum(n)*100)
item_and_hero_1 <- item_and_hero %>% filter(percentage > 5)
ggplot(na.omit(item_and_hero_1),aes(x=Class,y=percentage,fill=item_1)) + geom_bar(stat='identity',position='dodge')
```
```{r 15,echo=FALSE}
item_and_hero <- players_csv2 %>% group_by(Class,item_2) %>% tally() 
item_and_hero <- item_and_hero %>% group_by(Class) %>% mutate(percentage=n/sum(n)*100)
item_and_hero_1 <- item_and_hero %>% filter(percentage > 5)
ggplot(na.omit(item_and_hero_1),aes(x=Class,y=percentage,fill=item_2)) + geom_bar(stat='identity',position='dodge')
```

```{r 16,echo=FALSE}
item_and_hero <- players_csv2 %>% group_by(Class,item_3) %>% tally() 
item_and_hero <- item_and_hero %>% group_by(Class) %>% mutate(percentage=n/sum(n)*100)
item_and_hero_1 <- item_and_hero %>% filter(percentage > 5)
ggplot(na.omit(item_and_hero_1),aes(x=Class,y=percentage,fill=item_3)) + geom_bar(stat='identity',position='dodge')
```

```{r 17,echo=FALSE}
item_and_hero <- players_csv2 %>% group_by(Class,item_4) %>% tally() 
item_and_hero <- item_and_hero %>% group_by(Class) %>% mutate(percentage=n/sum(n)*100)
item_and_hero_1 <- item_and_hero %>% filter(percentage > 5)
ggplot(na.omit(item_and_hero_1),aes(x=Class,y=percentage,fill=item_4)) + geom_bar(stat='identity',position='dodge')
```
```{r 18,echo=FALSE}
item_and_hero <- players_csv2 %>% group_by(Class,item_4) %>% tally() 
item_and_hero <- item_and_hero %>% group_by(Class) %>% mutate(percentage=n/sum(n)*100)
...
```
```{r 19,echo=FALSE}
item_and_hero <- players_csv2 %>% group_by(Class,item_5) %>% tally() 
item_and_hero <- item_and_hero %>% group_by(Class) %>% mutate(percentage=n/sum(n)*100)
item_and_hero_1 <- item_and_hero %>% filter(percentage > 5)
ggplot(na.omit(item_and_hero_1),aes(x=Class,y=percentage,fill=item_5)) + geom_bar(stat='identity',position='dodge')
```